...
namespace gazebo
{
  ...
  class TestforcePlugin : public WorldPlugin
  {
    ...
    public: virtual void Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
    {
     ...
     // this->model=_model;
     this->toplink=this->model_child->GetLink	(	"ball_4"	);
     this->toprod0=this->logger0_model_base->GetLink	(	"logger-chassis"	);
     this->toprod1=this->logger1_model_base->GetLink	(	"logger-chassis"	);
     this->toprod2=this->logger2_model_base->GetLink	(	"logger-chassis"	);
     this->toprod3=this->logger3_model_base->GetLink	(	"logger-chassis"	);
     //this->toplink->SetForce(	this->force0);
    //UAV
    this->drone1_model = this->world->ModelByName("drone1");
    this->toprod4=this->logger0_model_base->GetLink	(	"base_link"	);
  ...
  public: void ApplyForce4(const double &_force4)
    {
      ...
      this->force3=this->CalForce(_force4,this->pos,this->bot_position4);
      //display the Length of force on the terminal
...
private: void QueueThread()
{
  ...
  private:
    ...
    //parameters of 8 cable nodes' position
       ...
       //末端执行器链接节点在其自身坐标中的位置
       double const posx1=0.0, posy1=0,posz1=0;
