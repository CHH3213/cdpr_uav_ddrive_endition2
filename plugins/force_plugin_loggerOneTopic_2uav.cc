...
//    &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    public: void ApplyForce0(const double &_force0,const double &_force1,const double &_force2,const double &_force3)
    {
      //calculate the force
...
public: void OnRosMsg0(const std_msgs::Float32MultiArrayConstPtr &_msg)
{
    ...
    this->ApplyForce0(_msg->data[0], _msg->data[1], _msg->data[2], _msg->data[3]);
    std::cerr <<"force_logger0: "<<_msg->data[0]<<'\n'<<'\n';
    std::cerr <<"force_logger1: "<<_msg->data[1]<<'\n'<<'\n';
    std::cerr <<"force_logger2: "<<_msg->data[2]<<'\n'<<'\n';
    std::cerr <<"force_logger3: "<<_msg->data[3]<<'\n'<<'\n';
//    std::cerr <<"logger: "<<_msg->data[4]<<'\n';
    std::cerr <<"======================="<<'\n';
...
//       double const posx1=0.0, posy1=0.0,posz1=0.1;//圆锥半径是0.05，高度是0.1时
       double const posx1=0.0, posy1=0.0,posz1=0.05;//球半径是0.05，圆心在中心
       //无人机端链接节点在其自身坐标中的位置
