...
class CDPR(gym.Env):
    def __init__(self):
        ...
        self.local_pos_sub3 = rospy.Subscriber(
            "/drone3/mavros/local_position/pose", PoseStamped, self.lp_cb3, queue_size=1)
        ...
        self.rc_out3 = rospy.Subscriber(
            "/drone3/mavros/rc/out", RCOut, self.rc_cb3, queue_size=1)
        ...
        self.acutator_control_pub3 = rospy.Publisher(
            "/drone3/mavros/actuator_control", ActuatorControl, queue_size=1)
        ...
        self.setpoint_raw_pub3 = rospy.Publisher(
            "/drone3/mavros/setpoint_raw/attitude", AttitudeTarget, queue_size=1)  # NED
        ...
        self.setposition_raw_pub3 = rospy.Publisher(
            "/drone3/mavros/setpoint_raw/local", PositionTarget, queue_size=1)  # NED
        ...
        self.local_pos_pub3 = rospy.Publisher(
            "/drone3/mavros/setpoint_position/local", PoseStamped, queue_size=1)
        ...
        self.thrust_pub3 = rospy.Publisher(
            "/drone3/mavros/setpoint_attitude/thrust", Thrust, queue_size=1)
        ...
        self.rc_override3 = rospy.Publisher(
            "/drone3/mavros/rc/override", OverrideRCIn, queue_size=1)

        self.force_publisher0 = rospy.Publisher(
            '/loggerAll/testforce', Float32MultiArray, queue_size=1)

        self.force_publisher4 = rospy.Publisher(
            '/drone1/testforce', Float32MultiArray, queue_size=1)
        self.force_publisher5 = rospy.Publisher(
            '/drone2/testforce', Float32MultiArray, queue_size=1)
        self.force_publisher6 = rospy.Publisher(
            '/drone3/testforce', Float32MultiArray, queue_size=1)
        ...
        self.arming_client3 = rospy.ServiceProxy(
            'drone3/mavros/cmd/arming', CommandBool)
        ...
        self.takeoff_service3 = rospy.ServiceProxy(
            '/drone3/mavros/cmd/takeoff', CommandTOL)
        ...
        self.set_mode_client3 = rospy.ServiceProxy(
            'drone3/mavros/set_mode', SetMode)
    ...
    def run_xyz(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.thrust1 = 0.01894 * \
                (self.rc_states1.channels[0] + self.rc_states1.channels[1] +
                 self.rc_states1.channels[2] + self.rc_states1.channels[3]) - 99.63
            self.thrust2 = 0.01894 * \
                (self.rc_states2.channels[0] + self.rc_states2.channels[1] +
                 self.rc_states2.channels[2] + self.rc_states2.channels[3]) - 99.63
            ...
            [self.drone1_position, _, _, _] = self.get_state(
                self.model_states, model_name[4])
            [self.drone2_position, _, _, _] = self.get_state(
            ...
            force_payload2drone1 = self.Coeff_elasticity * \
                (self.dist_cable1 - 2) * 100
            force_payload2drone2 = self.Coeff_elasticity * \
                (self.dist_cable2 - 2) * 100
            ...
            force_logger0, force_logger1, force_logger2, force_logger3 = self.compute_LoggerForce(
                force_payload2drone1/100, force_payload2drone2/100, force_payload2drone2/100, self.payload_gravity, self.drone_gravity, pid)
            ########################################################
            s = time.time()
            ...
            if self.drone1_position[2] > 0 and self.drone2_position[2] > 0 and self.payload_position[2] > 0.5:
                multiarray0.data = [force_logger0*100, force_logger1 *
                                    100, force_logger2*100, force_logger3*100, count4]
            ...
            if count_draw == 1100:
                ...
                plt.legend(labels=['logger0', 'logger1', 'logger2',
                           'logger3', 'drone1', 'drone2'], loc='best')
                plt.savefig(
                    '/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/force_2dSA.png')
                count_draw = 1
                plt.figure(2)
                ...
                plt.savefig(
                    '/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/PayloadPos_2dSA.png')
                plt.figure(3)
                plt.title('drone_pos')
                ...
                plt.savefig(
                    '/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/DronePos_2dSA.png')
                plt.show()
                break
        ...
        targetForcex = np.mean([self.target_loggerAll[i][0]
                               for i in range(count_draw-100, count_draw)])
        targetForcey = np.mean([self.target_loggerAll[i][1]
                               for i in range(count_draw-100, count_draw)])
        targetForcez = np.mean([self.target_loggerAll[i][2]
                               for i in range(count_draw-100, count_draw)])
        ...
        norm_thrust = np.linalg.norm(thrust1)+np.linalg.norm(thrust2)
        print("thrust_norm", norm_thrust)
        print(np.array(f_sum2)+np.array(f_sum1))
    ...
    def compute_LoggerForce(self, cableForce1, cableForce2, cableForce3, payload_gravity, drone_gravity, pid):
        ...
        cos_theta = np.sum([a*b for a, b in zip(real_cable_force1,
                           delta_load2drone1)])/(np.linalg.norm(real_cable_force1))
        ...
        force_valueLogger0, force_valueLogger1, force_valueLogger2, force_valueLogger3 = x[
            0], x[1], x[2], x[3]
        value = force_valueLogger0 * delta_load2logger0 + force_valueLogger1 * delta_load2logger1 + \
            force_valueLogger2 * delta_load2logger2 + force_valueLogger3 * \
            delta_load2logger3 - target_forceLogger0123
    ...
    def compute_force(self, throttle1, throttle2, throttle3, payload_gravity, drone_gravity, pid):
        ...
        [self.logger3_position, _, _, _] = self.get_state(
            self.model_states, model_name[3])
        # 使用mavros获取无人机位置信息
        [self.drone1_position, self.drone2_position, self.drone3_position,
            ori_drone1, ori_drone2, ori_drone3] = self.get_posByMavros()
        [self.drone1_position, _, _, _] = self.get_state(
            self.model_states, model_name[4])
        [self.drone2_position, _, _, _] = self.get_state(
            self.model_states, model_name[5])
        ...
        # 负载与小车间的力的方向,指向小车
        delta_load2logger0 = [self.logger0_position[0] - self.payload_position[0], self.logger0_position[1] -
                              self.payload_position[1], self.logger0_position[2]-(self.payload_position[2])]
        delta_load2logger1 = [self.logger1_position[0] - self.payload_position[0], self.logger1_position[1] -
                              self.payload_position[1], self.logger1_position[2]-(self.payload_position[2])]
        delta_load2logger2 = [self.logger2_position[0] - self.payload_position[0], self.logger2_position[1] -
                              self.payload_position[1], self.logger2_position[2]-(self.payload_position[2])]
        delta_load2logger3 = [self.logger3_position[0] - self.payload_position[0], self.logger3_position[1] -
                              self.payload_position[1], self.logger3_position[2]-(self.payload_position[2])]
        ...
        # 绳子方向,指向负载
        delta_load2drone1 = [self.payload_position[0]-self.drone1_position[0], self.payload_position[1] -
                             self.drone1_position[1], self.payload_position[2]-self.drone1_position[2]]
        delta_load2drone2 = [self.payload_position[0]-self.drone2_position[0], self.payload_position[1] -
                             self.drone2_position[1], self.payload_position[2]-self.drone2_position[2]]
        ...
        # 推力向量
        thrust_drone1 = np.array(
            [matrix1[0][2], matrix1[1][2], matrix1[2][2]])*throttle1
        thrust_drone2 = np.array(
            [matrix2[0][2], matrix2[1][2], matrix2[2][2]])*throttle2
        # thrust_drone3 = np.array([matrix3[0][2], matrix3[1][2], matrix3[2][2]])*throttle3
        print('thrust_drone1', thrust_drone1)
        #绳子与重力方向夹角,向量积的形式计算:a*b=|a||b|cos theta，单位是弧度
        payload_gravityDir = [0, 0, -payload_gravity]
        theta_cableGravity1 = np.arccos(
            delta_load2drone1[2]*payload_gravityDir[2]/(1*payload_gravity))
        theta_cableGravity2 = np.arccos(
            delta_load2drone2[2]*payload_gravityDir[2]/(1*payload_gravity))
        ...
        # 绳子方向上受的合力大小
        force_cable1 = drone_gravity * \
            np.cos(theta_cableGravity1) - throttle1 * \
            np.cos(theta_cableThrottle1)
        force_cable2 = drone_gravity * \
            np.cos(theta_cableGravity2) - throttle2 * \
            np.cos(theta_cableThrottle2)
        # force_cable3 = drone_gravity*np.cos(theta_cableGravity3) - throttle3 *np.cos(theta_cableThrottle3)

        force_cable1 = drone_gravity * \
            np.cos(theta_cableGravity1) - throttle1 * \
            np.cos(theta_cableGravity1)
        force_cable2 = drone_gravity * \
            np.cos(theta_cableGravity2) - throttle2 * \
            np.cos(theta_cableGravity2)
    ...
    def get_dist2(self):
        ...
        self.dt = time.time() - self.start
        self.payload_pose.position.x = self.payload_pose.position.x + \
            payload_twist.linear.x * self.dt
        self.payload_pose.position.y = self.payload_pose.position.y + \
            payload_twist.linear.y * self.dt
        self.payload_pose.position.z = self.payload_pose.position.z + \
            payload_twist.linear.z * self.dt
        self.drone_pose1.position.x = self.drone_pose1.position.x + \
            drone_twist1.linear.x * self.dt
        self.drone_pose1.position.y = self.drone_pose1.position.y + \
            drone_twist1.linear.y * self.dt
        self.drone_pose1.position.z = self.drone_pose1.position.z + \
            drone_twist1.linear.z * self.dt
        self.drone_pose2.position.x = self.drone_pose2.position.x + \
            drone_twist2.linear.x * self.dt
        self.drone_pose2.position.y = self.drone_pose2.position.y + \
            drone_twist2.linear.y * self.dt
        self.drone_pose2.position.z = self.drone_pose2.position.z + \
            drone_twist2.linear.z * self.dt
        self.drone_pose3.position.x = self.drone_pose3.position.x + \
            drone_twist3.linear.x * self.dt
        self.drone_pose3.position.y = self.drone_pose3.position.y + \
            drone_twist3.linear.y * self.dt
        self.drone_pose3.position.z = self.drone_pose3.position.z + \
            drone_twist3.linear.z * self.dt
        self.logger_pose0.position.x = self.logger_pose0.position.x + \
            logger_twist0.linear.x * self.dt
        self.logger_pose0.position.y = self.logger_pose0.position.y + \
            logger_twist0.linear.y * self.dt
        self.logger_pose0.position.z = self.logger_pose0.position.z + \
            logger_twist0.linear.z * self.dt
    ...
    def get_dist(self):
        ...
        dist_cone2drone1 = np.sqrt(np.sum(np.square([payload_pose.position.x-drone_pose1.position.x,
                                   payload_pose.position.y-drone_pose1.position.y, payload_pose.position.z-drone_pose1.position.z])))
        dist_cone2drone2 = np.sqrt(np.sum(np.square([payload_pose.position.x-drone_pose2.position.x,
                                   payload_pose.position.y-drone_pose2.position.y, payload_pose.position.z-drone_pose2.position.z])))
        dist_cone2drone3 = np.sqrt(np.sum(np.square([payload_pose.position.x-drone_pose3.position.x,
                                   payload_pose.position.y-drone_pose3.position.y, payload_pose.position.z-drone_pose3.position.z])))
    ...
    def get_posByMavros(self):
        ...
        :return: 返回无人机的位置
        '''
        pos_drone1 = [self.local_position1.pose.position.x,
                      self.local_position1.pose.position.y, self.local_position1.pose.position.z]
        pos_drone2 = [self.local_position2.pose.position.x,
...
class apmPID:
    def __init__(self, target):
        ...
        self.control_x = PID(np.asarray(
            [0.125*x_p, 0.00*x_i, 0.03*x_d])*x, target[0], upper=25, lower=-25)  # control position x
        self.control_y = PID(np.asarray(
            [0.125*y_p, 0.00*y_i, 0.03*y_d])*y, target[1], upper=25, lower=-25)  # control position y
        self.control_z = PID(np.asarray(
            [0.10*z_p, 0.00*z_i, 0.115*z_d])*z, target[2], upper=10, lower=-10)  # control position z
