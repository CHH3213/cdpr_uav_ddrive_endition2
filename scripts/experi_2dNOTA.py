...
class CDPR(gym.Env):
    ...
    def run_xyz(self):
        ...
        target = [0,0,7-np.sqrt(25-0.35**2)]
        pid = apmPID(target)
        ...
        while not rospy.is_shutdown():
            ...
            if self.drone1_position[2] > 0 :
                multiarray4.data = [force_payload2drone1/4, force_payload2drone1, count1]  # 拉力大小
                count += 1
            ...
            if self.drone2_position[2] > 0 :
                multiarray5.data = [force_payload2drone2/4, force_payload2drone2, count2]  # 拉力大小
            ...
            if count_draw == 600:
                plt.figure(1)
                plt.title('force')
                plt.plot(self.data_force1[0:])
                plt.plot(self.data_force2[0:])
                plt.plot(self.data_force3[0:])
                plt.plot(self.data_force4[0:])
                plt.plot(self.data_droneforce1[0:])
                plt.plot(self.data_droneforce2[0:])
        ...
        sio.savemat(self.saveData_dir + '/2droneSAData.mat',
                    dict(state_drone1=self.state_drone1, state_drone2=self.state_drone2,
                         state_payload=self.state_payload, force_drone1=self.cable_drone1,
                         force_drone2=self.cable_drone2, force_logger0=self.force_logger0,
    ...
    def run_last(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.thrust1 = max(18.88, 0.01894 * (
                        self.rc_states1.channels[0] + self.rc_states1.channels[1] + self.rc_states1.channels[2] +
                        self.rc_states1.channels[3]) - 99.63)
            self.thrust2 = max(18.88, 0.01894 * (
                        self.rc_states2.channels[0] + self.rc_states2.channels[1] + self.rc_states2.channels[2] +
                        self.rc_states2.channels[3]) - 99.63)
            ...
            force_logger0, force_logger1, force_logger2, force_logger3 = self.compute_LoggerForce(
                force_payload2drone1 / 100, force_payload2drone2 / 100, force_payload2drone1 / 100,
                self.payload_gravity, self.drone_gravity, pid)
            ########################################################

            if self.payload_position[2] > 0.5:
                multiarray0.data = [force_logger0 * 100, force_logger1 * 100, force_logger2 * 100, force_logger3 * 100,
                                    count4]
            ...
            if count_draw == 1100:
                ...
                plt.legend(labels=['logger0', 'logger1', 'logger2', 'logger3', 'drone1', 'drone2'],
                           loc='best')
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/force_2dNOTA.png')
                count_draw = 1
                plt.figure(2)
                plt.title('PayloadPos_multi')
                plt.plot(self.dataPosx_payload)
                ...
                plt.legend(labels=['pos_x', 'pos_y', 'pos_z'], loc='best')
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/PayloadPos_2dNOTA.png')
                plt.show()
                break
            ...
            count_draw += 1
            self.start = time.time()
        targetForce = [np.linalg.norm(self.target_loggerAll[i]) for i in range(count_draw-100,count_draw)]
        print('target', np.mean(targetForce))
        sio.savemat(self.saveData_dir + '/2droneNOTAData.mat',
    ...
    def compute_force(self,throttle1,throttle2,throttle3, payload_gravity,drone_gravity, pid):
        ...
        # 推力向量
        thrust_drone1 = throttle1*ori_drone1
        thrust_drone2 = throttle2*ori_drone2
        thrust_drone3 = throttle3*ori_drone3
...
class apmPID:
    def __init__(self,target):
        ...
        # self.control_z = PID(np.asarray([0.11*z_p, 0.0011*z_i, 0.011*z_d])*z, target[2], upper=0, lower= -1)  # control position z
        ...
        # self.control_z = PID(np.asarray([0.08*z_p, 0.00*z_i, 0.0038*z_d])*z, target[2], upper=10, lower= -10)  # control position z
        ...
        self.control_x = PID(np.asarray([0.25*x_p, 0.00*x_i, 0.025*x_d])*x, target[0], upper=25, lower=-25)  # control position x
        self.control_y = PID(np.asarray([0.25*y_p, 0.00*y_i, 0.025*y_d])*y, target[1], upper=25, lower=-25)  # control position y
        self.control_z = PID(np.asarray([0.018*z_p, 0.00*z_i, 0.037*z_d])*z, target[2], upper=10, lower= -10)  # control position z
