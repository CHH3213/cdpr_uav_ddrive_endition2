...
class CDPR(gym.Env):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            [self.payload_position, payload_orientation, payload_linear, payload_angular] = self.get_state(self.model_states,  model_name[7])
            [self.logger0_position, logger0_orientation, logger0_linear, logger0_angular] = self.get_state(self.model_states,  model_name[0])
            [self.logger1_position, logger1_orientation, logger1_linear, logger1_angular] = self.get_state(self.model_states,  model_name[1])
            [self.logger2_position, logger2_orientation, logger2_linear, logger2_angular] = self.get_state(self.model_states,  model_name[2])
            [self.logger3_position, logger3_orientation, logger3_linear, logger3_angular] = self.get_state(self.model_states,  model_name[3])
            [self.drone1_position, drone1_orientation, drone1_linear, drone1_angular] = self.get_state(self.model_states,  model_name[4])
            #############################################################################################################
            # self.payload_position = [self.payload_pose.position.x,self.payload_pose.position.y,self.payload_pose.position.z]
            # self.logger0_position = [self.logger_pose0.position.x,self.logger_pose0.position.y,self.logger_pose0.position.z]
            # self.drone1_position = [self.drone_pose1.position.x,self.drone_pose1.position.y,self.drone_pose1.position.z]
            ############################################################################################################

            force0_error, force1_error,force2_error = pid.cal_actions(self.payload_position)
            #
            # delta_load2logger0 = [self.payload_position[0]-self.logger0_position[0],self.payload_position[1]-self.logger0_position[1],self.payload_position[2]-self.logger0_position[2]]
            # delta_load2logger1 = [self.payload_position[0]-self.logger1_position[0],self.payload_position[1]-self.logger1_position[1],self.payload_position[2]-self.logger1_position[2]]
            # delta_load2logger2 = [self.payload_position[0]-self.logger2_position[0],self.payload_position[1]-self.logger2_position[1],self.payload_position[2]-self.logger2_position[2]]
            # delta_load2logger3 = [self.payload_position[0]-self.logger3_position[0],self.payload_position[1]-self.logger3_position[1],self.payload_position[2]-self.logger3_position[2]]
            ...
            # theta_load2logger0 = self.payload_position[2]/np.sqrt(np.sum(np.square(delta_load2logger0)))
            # theta_load2drone1 = self.payload_position[2]/np.sqrt(np.sum(np.square(delta_load2drone1)))
            ...
            print('payload_position_z',self.payload_position[2]-3)
            ...
            self.unpausePhysics()
            if drone1_fly and self.payload_position[2]>0.1:
    ...
    def run_xyz(self):
        ...
        while not rospy.is_shutdown():
            ...
            if np.abs(self.drone1_position[2]-7.5)<=0.5 :
                self.set_mode_client1(custom_mode="ALT_HOLD")
            if np.abs(self.drone2_position[2]-7.5)<=0.5 :
                self.set_mode_client2(custom_mode="ALT_HOLD")
            if np.abs(self.drone3_position[2]-7.5)<=0.5 :
            ...
            # pose = Pose()
            # if count == 1:
            #     self.init_posdrone1 = self.drone1_position
            ...
            # print(self.payload_linear[2]<0)
            # print('>2', self.payload_position[2]>2.5)
            self.Coeff_elasticity = 1
            # 重物与绳子间拉力
            # force_payload2cable1 = self.payload_gravity*100
            # self.force_cable1 = self.force_cable1+self.Coeff_elasticity*(self.dist_load2drone1-5)* int(self.dt*1000+1)
            force_payload2cable1 = self.Coeff_elasticity * (self.dist_load2drone1 - 5) * 100
            force_payload2drone1 =  self.Coeff_elasticity * (self.dist_load2drone1 - 5) * 100
            force_payload2cable1 = self.force_cable1
            force_payload2drone1 = self.force_cable1 + self.Coeff_elasticity * (self.dist_load2drone1 - 5) * 100

            # force_payload2cable2 = self.payload_gravity*100
            force_payload2cable2 = self.Coeff_elasticity * (self.dist_load2drone2 - 5) * 100
            force_payload2drone2 =  self.Coeff_elasticity * (self.dist_load2drone2 - 5) * 100
            force_payload2cable2 = self.force_cable2
            force_payload2drone2= self.force_cable2+self.Coeff_elasticity*(self.dist_load2drone2-5)* 100


            # force_payload2cable3 = self.payload_gravity*100
            force_payload2cable3 =  self.Coeff_elasticity * (self.dist_load2drone3 - 5) * 100
            force_payload2drone3 =  self.Coeff_elasticity * (self.dist_load2drone3 - 5) * 100
            force_payload2cable3 = self.force_cable3
            force_payload2drone3 = self.force_cable3 + self.Coeff_elasticity * (self.dist_load2drone3 - 5) * 100
            # drone1
            if self.drone1_position[2]>0  and self.dist_load2drone1>=5:
                multiarray4.data = [force_payload2drone1, force_payload2cable1,count1] # 拉力大小
                # self.force_publisher4.publish(multiarray4)
                flag =True
                count += 1
                flag_drone1 = True
            # # elif flag_drone1 and self.drone1_position[2] > 0 and self.dist_load2drone1 < 5 and self.payload_linear[2] < 0:
            elif count>1 and self.drone1_position[2] > 0 and self.dist_load2drone1 < 5 and self.payload_linear[2] < 0:
                # multiarray4.data = [0, 0.25 * 9.8 * 100+self.Coeff_elasticity*(self.dist_load2drone1-5)*100]
                multiarray4.data = [0, (0.25 * 9.8 * 100+np.abs(self.payload_linear[2])/self.dt*0.25)/1, count1]
                flag = False
                flag_drone1 = False
                # print('hahahha1')
            else:
            ...
            # drone2
            if self.drone2_position[2]>0  and self.dist_load2drone2>=5:
                multiarray5.data = [force_payload2drone2, force_payload2cable2,count2] # 拉力大小
                # self.force_publisher4.publish(multiarray4)
                flag =True
                flag_drone2 = True
                count += 1
            # elif flag_drone2 and self.drone2_position[2] > 0 and self.dist_load2drone2 < 5 and self.payload_linear[2] < 0:
            elif count>1 and self.drone2_position[2] > 0 and self.dist_load2drone2 < 5 and self.payload_linear[2] < 0:
                # multiarray4.data = [0, 0.25 * 9.8 * 100+self.Coeff_elasticity*(self.dist_load2drone1-5)*100]
                multiarray5.data = [0, (0.25 * 9.8 * 100+np.abs(self.payload_linear[2])/self.dt*0.25)/1 ,count2]
                flag = False
                flag_drone2 = False
                # print('hahahha2')
            else:
            ...
            # drone3
            if self.drone3_position[2]>0  and self.dist_load2drone3>=5:
                multiarray6.data = [force_payload2drone3, force_payload2cable3,count3] # 拉力大小
                # self.force_publisher4.publish(multiarray4)
                flag =True
                count += 1
                flag_drone3 = True
            # elif flag_drone3 and self.drone3_position[2] > 0 and self.dist_load2drone3 < 5 and self.payload_linear[2] < 0:
            elif count>1 and self.drone3_position[2] > 0 and self.dist_load2drone3 < 5 and self.payload_linear[2] < 0:
                # multiarray6.data = [0, 0.25 * 9.8 * 100+self.Coeff_elasticity*(self.dist_load2drone1-5)*100]
                multiarray6.data = [0, (0.25 * 9.8 * 100+np.abs(self.payload_linear[2])/self.dt*0.25)/1 ,count3]
                flag = False
                # print('hahahha3')
                flag_drone3 = False
            else:
            ...
            # if count>1 and  self.dist_load2drone3 < 5 or self.dist_load2drone2 < 5 or self.dist_load2drone1 < 5:
            #     multiarray4.data = [0, force_payload2drone1]
            #     multiarray5.data = [0, force_payload2drone2]
            #     multiarray6.data = [0, force_payload2drone3]
            count1+=1
            self.force_publisher4.publish(multiarray4)
            time.sleep(0.0015)
            count2+=1
            ...
            # 原来force_plugin.cc插件，4量小车分别4个话题
            if self.drone2_position[2]>0  and self.drone2_position[2]>0 and self.drone3_position[2]>0 and self.payload_position[2]>0.5 :
                multiarray0.data = [force_logger0*100,count4]
                multiarray1.data = [force_logger1*100,count5]
                multiarray2.data = [force_logger2*100,count6]
                multiarray3.data = [force_logger3*100,count7]
            else:
                multiarray0.data = [0, count4]
                multiarray1.data = [0, count5]
                multiarray2.data = [0, count6]
                multiarray3.data = [0, count7]
            count6+=1
            self.force_publisher2.publish(multiarray2)
            time.sleep(0.0045)
            count5+=1
            ...
            # 保存数据
            self.data_droneforce1.append(force_payload2cable1)
            self.data_droneforce2.append(force_payload2cable2)
            ...
            if count_draw == 500:
                ...
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/force_multi.png')
                count_draw = 1
                plt.figure(2)
                ...
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/PayloadPos_multi.png')
                plt.show()
                break
    ...
    def compute_force(self,throttle1,throttle2,throttle3, payload_gravity,drone_gravity, pid):
        ...
        # self.force_cable2 = np.abs(force_cable2)* int(self.dt*1000+1)
        # self.force_cable3 = np.abs(force_cable3)* int(self.dt*1000+1)
        ...
        # print('self.payload_position',self.payload_position)
        # 目标小车合力
        # force_pid = [0,0,0]
        # 多架无人机
        ...
        # print('heli', force_cableDrone1 + force_cableDrone2 + force_cableDrone3 + payload_gravityDir)
        ...
        # print('value',value)
        # self.targetValue.append(result*100)
    ...
    def run_logger(self):
        ...
        dt = 0.01
        while not rospy.is_shutdown():
            # self.dist_load2drone1, drone1_fly, self.dist_load2drone2, drone2_fly, self.dist_load2drone3, drone3_fly = self.get_dist()
            # print('distance',self.dist_load2drone1,self.dist_load2drone2,self.dist_load2drone3)
            #
            # model_name = ['logger0', 'logger1', 'logger2', 'logger3', 'drone1', 'drone2', 'drone3', 'payload']
            # [self.payload_position, _, _, _] = self.get_state(self.model_states, model_name[7])
            # [self.drone1_position, _, _, _] = self.get_state(self.model_states,  model_name[4])
            # [self.drone2_position, _, _, _] = self.get_state(self.model_states,  model_name[5])
            # [self.drone3_position, _, _, _] = self.get_state(self.model_states,  model_name[6])
            # print('self.drone1_position',self.drone1_position)
            # print('self.drone2_position',self.drone2_position)
            # print('self.drone3_position',self.drone3_position)
            target = [0, 0, 2.5]
            pid = apmPID(target)
            force_logger0,force_logger1,force_logger2,force_logger3 = self.compute_force(22,22,22,self.payload_gravity,self.drone_gravity,pid)
            start = time.time()
            ...
            #     time.sleep(3)
            #     count += 1
            # multiarray4.data = [0.25*10*100,0.25*10*100]
            # self.force_publisher4.publish(multiarray4)
            print(self.payload_linear[2]>0)
            ...
            if self.drone1_position[2]>0  and self.dist_load2drone1>=5:
                ...
                multiarray4.data = [0, 0.25 * 9.8 * 100]
                self.force_publisher4.publish(multiarray4)

            print('dist_load2drone1', self.dist_load2drone1)

            # if self.drone1_position[2]>0 and self.payload_position[2]>0.5:
...
class apmPID:
    def __init__(self,target):
        ...
        # ## 1 20N推力，小于5米时要/3
        self.control_x = PID([0.25, 0.00, 0.001], target[0], upper=2.5, lower=-2.5)  # control position x
        self.control_y = PID([0.25, 0.00, 0.001], target[1], upper=2.5, lower=-2.5)  # control position y
        self.control_z = PID([0.1, 0.00, 0.001], target[2], upper=0, lower= -1)  # control position z
