...
class CDPR(gym.Env):
    ...
    def run_last(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if count_draw>100:
            #     self.drone1_position[2] = self.altitude
            #     self.drone2_position[2] = self.altitude
            ...
            if self.payload_position[2] > 0.1:
            ...
            if count_draw == 1100:
                ...
                plt.legend(labels=['logger0', 'logger1', 'logger2', 'logger3', 'drone1', 'drone2'],
                           loc='best')
                plt.savefig(
                    '/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/Data/figures/force_2dSA.png')
                count_draw = 1
                plt.figure(2)
                ...
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/Data/figures/PayloadPos_2dSA.png')
                plt.figure(3)
                plt.title('drone_pos')
                plt.plot(self.dataPosz_drone1)
                plt.plot(self.dataPosz_drone2)
                plt.axhline(y=self.altitude, color='r', linestyle='-')
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/Data/figures/DronePos_2dSA.png')
                
                plt.figure(4)
                plt.title('dt')
                plt.plot(self.dt_draw)
                plt.axhline(y=0.1, color='r', linestyle='-')
                # plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/Data/figures/dt_2dSA.png')
    ...
    def get_posByMavros(self):
        ...
        :return: 返回无人机的位置
        '''
        pos_drone1 = [self.local_position1.pose.position.x-0.5,self.local_position1.pose.position.y,self.local_position1.pose.position.z]
        pos_drone2 = [self.local_position2.pose.position.x+0.5,self.local_position2.pose.position.y,self.local_position2.pose.position.z]
...
class apmPID:
    def __init__(self,target):
        ...
        z_p,z_i,z_d = 1,1,0.5
        x,y,z = 5,5,2.5
        self.control_x = PID(np.asarray([0.25*x_p, 0.00*x_i, 0.025*x_d])*x, target[0], upper=250, lower=-250)  # control position x
        self.control_y = PID(np.asarray([0.25*y_p, 0.00*y_i, 0.025*y_d])*y, target[1], upper=250, lower=-250)  # control position y
        self.control_z = PID(np.asarray([0.04*z_p, 0.00*z_i, 0.08*z_d])*z, target[2], upper=100, lower= -100)  # control position z
