...
class CDPR(gym.Env):
    ...
    def run_xyz(self):
        ...
        while not rospy.is_shutdown():
            ...
            # # 第二种绳子模型时，弹性系数5比较合适。
            # force_payload2cable1 = self.Coeff_elasticity * (self.dist_load2drone1 - 7) * 100
            # force_payload2drone1 =  self.Coeff_elasticity * (self.dist_load2drone1 - 7) * 100
            # force_payload2cable1 = self.force_cable1
            # # 2
            # force_payload2drone1 = self.force_cable1 + self.Coeff_elasticity * (self.dist_load2drone1 - 7) * 100
            # force_payload2cable11 = self.force_cable1 / 5 * self.dist_load2drone1
            # force_payload2cable11 = self.force_cable1 - self.Coeff_elasticity * (self.dist_load2drone1 -7) * 100
            # if force_payload2drone1 < 0:
            #     force_payload2drone1 = 0
            # force_payload2cable2 = self.Coeff_elasticity * (self.dist_load2drone2 - 7) * 100
            # force_payload2drone2 =  self.Coeff_elasticity * (self.dist_load2drone2 - 7) * 100
            ...
            # force_payload2drone2= self.force_cable2+self.Coeff_elasticity*(self.dist_load2drone2-7)* 100
            # if force_payload2drone2 < 0:
            #     force_payload2drone2 = 0
            #
            # # force_payload2cable3 = self.payload_gravity*100
            # force_payload2cable3 =  self.Coeff_elasticity * (self.dist_load2drone3 - 7) * 100
            # force_payload2drone3 =  self.Coeff_elasticity * (self.dist_load2drone3 - 7) * 100
            ...
            # force_payload2drone3 = self.force_cable3 + self.Coeff_elasticity * (self.dist_load2drone3 - 7) * 100
            ...
            #     multiarray4.data = [force_payload2cable1, force_payload2cable1, count1]  # 拉力大小
            #     count += 1
            # else:
            ...
            #     multiarray5.data = [force_payload2cable2, force_payload2cable2, count2]  # 拉力大小
            # else:
            #     multiarray5.data = [0, 0, count2]
            #     flag = False
            # # # drone3
            # if self.drone3_position[2] > 0 :
            #     multiarray6.data = [force_payload2cable2, force_payload2cable2, count3]  # 拉力大小
            #
            # else:
            ...
            if count_draw == 1100:
                ...
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/force_2dBA.png')
                count_draw = 1
                plt.figure(2)
                ...
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/PayloadPos_2dBA.png')
                plt.figure(3)
                plt.title('drone_pos')
                plt.plot(self.dataPosz_drone1)
                plt.plot(self.dataPosz_drone2)
                plt.axhline(y=7.5, color='r', linestyle='-')
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/DronePos_2dBA.png')
                plt.show()
                break
        ...
        sio.savemat(self.saveData_dir + '/2droneBAData_v23.mat',
...
class apmPID:
    def __init__(self,target):
        ...
        self.control_z = PID(np.asarray([0.1049*z_p, 0.00*z_i, 0.0675*z_d])*z, target[2], upper=10, lower= -10)  # control position z
