...
class CDPR(gym.Env):
    ...
    def run_last(self):
        ...
        pose_stamp1.pose.position.z = 10.5
        pose_stamp2.pose.position.x = 0.0
        pose_stamp2.pose.position.y = 0.0
        pose_stamp2.pose.position.z = 10.5
        self.resetPayload()
        while not rospy.is_shutdown():
            ...
            self.takeoff_service1(altitude=10.5)
            self.takeoff_service2(altitude=10.5)
    ...
    def get_posByMavros(self):
        ...
        :return: 返回无人机的位置
        '''
        pos_drone1 = [self.local_position1.pose.position.x-3.0,
                      self.local_position1.pose.position.y, self.local_position1.pose.position.z]
        pos_drone2 = [self.local_position2.pose.position.x+3.0,
...
class apmPID:
    def __init__(self, target):
        ...
        self.control_x = PID(np.asarray(
            [0.25*x_p, 0.00*x_i, 0.025*x_d])*x, target[0], upper=250, lower=-250)  # control position x
        self.control_y = PID(np.asarray(
            [0.25*y_p, 0.00*y_i, 0.025*y_d])*y, target[1], upper=250, lower=-250)  # control position y
        self.control_z = PID(np.asarray(
            [0.04*z_p, 0.00*z_i, 0.08*z_d])*z, target[2], upper=100, lower=-100)  # control position z
