...
class CDPR(gym.Env):
    def __init__(self):
        ...
        self.local_pos_sub1 = rospy.Subscriber("/drone1/mavros/local_position/pose", PoseStamped, self.lp_cb, queue_size=1)
        # self.local_pos_sub2 = rospy.Subscriber("/drone2/mavros/local_position/pose", PoseStamped, self.lp_cb, queue_size=1)
        # self.local_pos_sub3 = rospy.Subscriber("/drone3/mavros/local_position/pose", PoseStamped, self.lp_cb, queue_size=1)
    ...
    def run_xyz(self):
        ...
        while not rospy.is_shutdown():
            ...
            if drone1_fly == True and dist_cone2drone1>=5:
                # if drone1_fly == True:
                #     multiarray4.data = [dist_cone2drone1, self.cable_length, self.Coeff_elasticity] # 距离，绳长，弹性系数
                self.force_cable1 = self.force_cable1+5*(dist_cone2drone1-5)*100
            ...
            # self.force_publisher0.publish(multiarray0)
            # if drone1_fly or drone2_fly or drone3_fly:
            #     multiarray0.data = [force_logger0*1]
            #     multiarray1.data = [force_logger1*1]
            #     multiarray2.data = [force_logger2*1]
            #     multiarray3.data = [force_logger3*1]
            #     self.force_publisher0.publish(multiarray0)
            #     self.force_publisher1.publish(multiarray1)
            #     self.force_publisher2.publish(multiarray2)
            #     self.force_publisher3.publish(multiarray3)
    ...
    def compute_force(self,throttle1,throttle2,throttle3, cable_gravity,drone_gravity, pid):
        ...
        # 目标小车合力
        # force_pid = [0,0,0]
        target_forceLogger0123 = force_pid -  force_cableDrone1 - force_cableDrone2 - force_cableDrone3 - [0,0,-cable_gravity]
...
class apmPID:
    def __init__(self,target):
        # 位置式
        self.control_x = PID_posi(0.3, 0, 0.001, target[0], up=1000, low=-1000)  # control position x
        self.control_y = PID_posi(0.3, 0., 0.002, target[1], up=1000, low=-1000)  # control position y
        self.control_z = PID_posi(0.9, 0.005, 0.01, target[2], up=1000, low= 0)  # control position z
        # 增量式
