...
class CDPR(gym.Env):
    def __init__(self):
        ...
        self.force_publisher = rospy.Publisher('/loggerAll/testforce', Float32MultiArray, queue_size=1)
    ...
    def run_xyz(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.Coeff_elasticity = 20
            # 重物与绳子间拉力
            # force_payload2cable = self.payload_gravity*100
            force_payload2cable = self.force_cable1 + self.Coeff_elasticity * (self.dist_load2drone1 - 5) * 100
            ...
            if self.drone1_position[2]>0  and self.dist_load2drone1>=5:
                multiarray4.data = [force_payload2drone1, force_payload2cable] # 拉力大小
                # self.force_publisher4.publish(multiarray4)
                flag =True
                count += 1
                ...
                multiarray4.data = [0,  0.25 * 9.8 * 100+np.abs(self.payload_linear[2])/self.dt*0.25]
                flag = False
            else:
                multiarray4.data = [0, 0]
                flag = False
            self.force_publisher4.publish(multiarray4)
            time.sleep(0.001)
            # self.pausePhysics()
            # if drone2_fly == True and self.dist_load2drone2>=5:
            #     # if drone2_fly == True:
            #     #     multiarray5.data = [self.dist_load2drone2, self.cable_length, self.Coeff_elasticity]
            #     force_payload2cable2 = self.force_cable2
            #     self.force_cable2 = self.force_cable2+self.Coeff_elasticity*(self.dist_load2drone2-5)*50
            #     multiarray5.data = [self.force_cable2,force_payload2cable2] # 拉力大小
            #     # self.unpausePhysics()
            #     self.force_publisher5.publish(multiarray5)
            #
            # # self.pausePhysics()
            # if drone3_fly == True and self.dist_load2drone3>=5:
            #     # if drone3_fly == True:
            #     #     multiarray6.data = [self.dist_load2drone3, self.cable_length, self.Coeff_elasticity]
            #     force_payload2cable3 = self.force_cable1
            #     self.force_cable3 = self.force_cable3+self.Coeff_elasticity*(self.dist_load2drone3-5)*50
            ...
            #     multiarray2.data = [force_logger2*100]
            #     multiarray3.data = [force_logger3*100]
            #     # multiarray0.data = [1.0*100]
            #     # multiarray1.data = [1.0*100]
            #     # multiarray2.data = [1.0*100]
            #     # multiarray3.data = [1.0*100]
            #     self.force_publisher0.publish(multiarray0)
            #     time.sleep(0.001)
            #     self.force_publisher1.publish(multiarray1)
            #     time.sleep(0.001)
            ...
            #     self.force_publisher3.publish(multiarray3)
            if self.drone1_position[2] > 0 and self.payload_position[2] > 0.5:
                multiarray.data = [force_logger0 * 100, force_logger1 * 100, force_logger2 * 100, force_logger3 * 100,
                                   0]
                self.force_publisher.publish(multiarray)
            ...
            self.start = time.time()
            # 保存数据
            self.dataPos_payload.append(self.payload_position[2])
            self.data_droneforce1.append(force_payload2cable)
            self.data_force1.append(force_logger0*1)
            self.data_force2.append(force_logger1*1)
            self.data_force3.append(force_logger2*1)
            self.data_force4.append(force_logger3*1)
            if count_draw == 500:
                plt.figure(1)
                plt.title('force')
                ...
                # plt.plot(self.targetValue)
                # plt.legend(labels = ['logger0','logger1','logger2','logger3','drone1','target'], loc='best')
                plt.legend(labels = ['logger0','logger1','logger2','logger3','drone1'], loc='best')
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/force_single.png')
                plt.figure(2)
                plt.title('payload_pos')
                plt.plot(self.dataPos_payload)
                plt.savefig('/home/chh3213/ros_wc/src/cdpr_uav_ddrive/scripts/Payload_single.png')
                plt.show()
                break
    ...
    def compute_force(self,throttle1,throttle2,throttle3, payload_gravity,drone_gravity, pid):
        ...
        # 一架无人机
        target_forceLogger0123 = force_pid -  force_cableDrone1 - payload_gravityDir
    ...
    def run_logger(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.drone1_position[2]>0 and self.payload_position[2]>0.5:
                multiarray.data = [force_logger0 * 100, force_logger1 * 100,force_logger2 * 100,force_logger3 * 100,0]
                self.force_publisher.publish(multiarray)
